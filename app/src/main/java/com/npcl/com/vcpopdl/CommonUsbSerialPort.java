...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
    ...
    protected abstract void openInt(UsbDeviceConnection connection) ;
    ...
    @Override
    public int read(final byte[] dest, final int timeout) {
        if(mConnection == null) {
    ...
    protected int readFilter(final byte[] buffer, int len)  { return len; }

    @Override
    public int write(final byte[] src, final int timeout)  {
        int offset = 0;
    ...
    public abstract EnumSet<ControlLine> getControlLines() ;

    public abstract EnumSet<ControlLine> getSupportedControlLines() ;

    @Override
    public boolean purgeHwBuffers(boolean purgeWriteBuffers, boolean purgeReadBuffers)  {
        return false;
    }
