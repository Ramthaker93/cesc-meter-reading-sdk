// VERSION: V68-SDK — LP PLAIN FULL-READ FALLBACK + HONEST PARTIAL/BILLING VERDICTS
// (07-08, evidence: field log SDK_OPTICAL_LOG_950010_Anshuman_Singh, meters KT339182 /
// KT326986 / KT247345 / KT349245, and EDIS meter KT329683).
//
...
// reports entries_in_use=0 for LP while plainly holding data, so the SDK could only guess
...
// skip. Same guard shape as day-probe (V60), entry-probe (V54), D2 compound (V62): a result
...
// VERSION: V59-SDK — EARLIER FULL-RECONNECT TRIGGER + CHEAPER GAPDRIVE LADDER (27-07, from
...
public class ReadingSDK {
    ...
    // =====================================================================
    // DLMS FIELDS (unchanged from Reading.java)
    // =====================================================================
    ...
    // (read-by-ENTRY-NUMBER) instead of selector 1 (time range). Set via lpGetByEntry() only.
    ...
    // Current active meter make
    private MeterMake currentMeterMake = MeterMake.SECURE;

    // HPL logical device name from OBIS 0.0.42.0.0.255
    private String hplLogicalDeviceName = "";

    // Last GetParameter_LS outcome
    ...
    // V68: cumulative LP record loss across all accepted pages this session — see lpAuditPage().
    ...
    private static final String SDK_VERSION = "V68";
    ...
    // CONSTRUCTOR
    // =====================================================================
    public ReadingSDK(Context context) {
    ...
    // =====================================================================
    // CALLBACK HELPERS — always dispatched on the main thread
    // =====================================================================
    private void fireProgress(ReadingCallback cb, String msg, int pct) {
        mainHandler.post(() -> { if (cb != null) cb.onProgress(msg, pct); });
    }
    private void fireError(ReadingCallback cb, String msg) {
        appendLog("ERROR: " + msg); flushLog();
        mainHandler.post(() -> { if (cb != null) cb.onError(msg); });
    }
    private void fireComplete(ReadingCallback cb, MeterReadingResult result) {
        flushLog();
        mainHandler.post(() -> { if (cb != null) cb.onComplete(result); });
    }

    // DB stubs (not needed in SDK — log only)
    private void UpdateStatus(String meterNo, String status) {
        appendLog("STATUS: " + meterNo + " — " + status);
    }
    ...
    // =====================================================================
    // MAIN READING LOGIC (extracted from Reading.java doInBackground)
    ...
    private void runReading(MeterMake meterMake, ReadingMode readingMode,
        ...
        try {
            ...
            if (availableDrivers.isEmpty()) {
                fireError(callback, "USB cable not detected. Please connect optical cable."); return;
            }
            UsbSerialDriver driver = availableDrivers.get(0);
                UsbDeviceConnection connection = manager.openDevice(driver.getDevice());
                if (connection == null) {
                    fireError(callback, "USB permission denied. Grant permission and retry."); return;
                }
                ...
                fireProgress(callback, "Sending SNRM (NRM)...", 15);
                ...
                if (!nrmOk) {
                    ...
                    fireProgress(callback, "NRM retry (cable alignment)...", 15);
                ...
                if (!nrmOk) {
                    ...
                    fireProgress(callback, "NRM retry 2 (check cable alignment)...", 15);
                ...
                if (abortRequested) {
                    fireProgress(callback, "WARN: Aborted by user.", 0);
                    return;
                }
                ...
                if (!nrmOk && currentMeterMake == MeterMake.LNT) {
                    ...
                    fireProgress(callback, "NRM probe: 2-byte server address (L&T NTD 3-phase)...", 15);
                    bytAddMode = 1;
                    AddressInit();
                    android.os.SystemClock.sleep(500);
                    nrmOk = SetNRM(port, bytWait, (byte) 2, bytTimOut);
                ...
                if (!nrmOk) {
                    fireError(callback, "NRM failed — check optical cable alignment on meter."); return;
                }
                ...
                if (aarqResult == 1) {
                    fireError(callback, "Authentication FAILED — wrong password or meter type mismatch."); return;
                } else if (aarqResult == 2) {
                    fireError(callback, "Communication error during AARQ — check cable."); return;
                }
                ...
                fireProgress(callback, "Reading Meter Number (OBIS 0000600100FF)...", 28);
                ...
                fireProgress(callback, "Reading Manufacturer (OBIS 0000600800FF)...", 32);
                ...
                if (mfgData != null && !mfgData.toString().isEmpty()) {
                    manufacturerStr = hexToString(mfgData.toString().replace("0A0A",""))
                            .replace("\b","").replace("\r\n","").replace("\n","").replace("\t","").trim();
                    fireProgress(callback, "Manufacturer: " + manufacturerStr, 34);
                }
                ...
                try {
                    ...
                    if (rtcData != null && rtcData.length() > 24) {
                        ...
                        fireProgress(callback, "WARN: Could not read RTC — proceeding anyway.", 32);
                    }
                } catch (Exception rtcEx) {
                    fireProgress(callback, "WARN: RTC check error: " + rtcEx.getMessage() + " — proceeding.", 32);
                }
                // ============================================================
                // END RTC CHECK
                // ============================================================
                ...
                if (sbNm != null && sbNm.length() > 0) {
                    ...
                    fireProgress(callback, "Name Plate read OK", 40);
                } else {
                    fireProgress(callback, "WARN: Name Plate read incomplete — continuing.", 40);
                }
                ...
                switch (readingMode) {
                    ...
                    case INSTANTANEOUS: {
                        ...
                        fireProgress(callback, "Instantaneous read OK (" + ((System.currentTimeMillis()-tInst0)/1000) + "s)", 80);
                        break;
                    }
                    ...
                    case BILLING: {
                        ...
                        MeterData.append(ReadInstantData(port));
                        fireProgress(callback, "✓ Instantaneous done (" + ((System.currentTimeMillis()-tBm_inst)/1000) + "s) — reading Billing...", 55);
                        UpdateStatus(CescRajMeterno, "Instantaneous OK");
                    ...
                    case COMPLETE: {
                        ...
                        fireProgress(callback, "Downloading Instantaneous data... (5-15s)", 30);
                        MeterData.append(ReadInstantData(port));
                        ...
                        if (System.currentTimeMillis() >= sessionDeadlineMs) {
                            ...
                            fireProgress(callback, "WARN: Session limit reached — skipping Billing+Midnight+Events+LP", 79);
                        } else {
                            // ── Phase 2: Billing ─────────────────────────────────────────
                            ...
                            if (billingData != null && billingData.length() > 0) {
                                fireProgress(callback, "✓ Billing done (" + (tBillElapsed/1000) + "s)", 46);
                                UpdateStatus(CescRajMeterno, "Billing OK");
                            } else {
                                phasePartialBilling = true;
                                fireProgress(callback, "WARN: Billing empty — new meter or no billing cycle yet (" + (tBillElapsed/1000) + "s)", 46);
                            ...
                            if (System.currentTimeMillis() >= sessionDeadlineMs) {
                                ...
                                fireProgress(callback, "WARN: Session limit reached — skipping Midnight+Events+LP", 79);
                            } else {
                                bytTimOut = fastTimOut;
                                bytTryCnt = fastTryCnt;
                                ...
                                if (System.currentTimeMillis() >= sessionDeadlineMs) {
                                    ...
                                    fireProgress(callback, "WARN: ⚠ Session limit reached — Events+LP skipped", 79);
                                } else {
                                    bytTimOut = fastTimOut;
                                    bytTryCnt = fastTryCnt;
                                    long tEvtStart = System.currentTimeMillis();
                                    fireProgress(callback, "Downloading Events... (10-20s)", 54);
                                    MeterData.append(ReadEventData(port));
                                    long tEvtElapsed = System.currentTimeMillis() - tEvtStart;
                                    fireProgress(callback, "✓ Events done (" + (tEvtElapsed/1000) + "s)", 58);
                                    ...
                                    if (readingMode == ReadingMode.BILLING_EVENT) {
                                        ...
                                        fireProgress(callback, "Load Profile skipped (Billing+Event mode)", 60);
                                    } else if (lpRemaining < 60000L) {
                                        appendLog("SESSION_SKIP_LP: only " + (lpRemaining/1000) + "s remaining — insufficient for LP");
                                        fireProgress(callback, "WARN: ⚠ Insufficient time for Load Profile — skipping", 60);
                                    } else {
                                        bytTimOut = (byte) 8;
                                        bytTryCnt = (byte) 2;
                                        ...
                                        fireProgress(callback, "Downloading Load Profile… (idle-abort after 45s silence)", 62);
                        ...
                        // V68: partial LP is otherwise invisible — pages that returned far fewer
                ...
                if (Filenm.isEmpty()) {
                    ...
                    if (MeterData.length() > 0) {
                        ...
                        fireProgress(callback, "Partial data saved: " + Filenm, 90);
                    } else {
                        fireError(callback, "No data downloaded. Check cable and meter connection."); return;
                    }
                }
            ...
            fireError(callback, "Reading error: " + ex.getMessage());
        }
    }
    ...
    private Map<String, int[]> buildScalerMap(String dataUpper) {
     ...
     */
    private Map<String, int[]> buildBillingScalerMap(String dataUpper) {
        Map<String, int[]> map = new HashMap<>();
        ...
        if (bspCoHex != null && bspValHex != null) {
            ...
            List<String> bspObis = new ArrayList<>();
    ...
    private String parseDlmsRegisterWithUnit(String dataUpper,
        ...
        try {
            ...
            if (uc == 0xFF && (obisUp.endsWith("0800FF") || obisUp.endsWith("0600FF"))) {
                ...
                Map<String, int[]> bspFallback = buildBillingScalerMap(dataUpper);
     ...
     */
    private Map<String, Integer> parseBillingCaptureObjects(String dataUpper) {
        ...
        Map<String, int[]> colMeta = new LinkedHashMap<>(); // OBIS → {col, classId, attrIdx}
        try {
            ...
            for (Map.Entry<String, int[]> e : colMeta.entrySet()) {
    ...
    private String extractBillingByObis(String dataUpper,
        ...
        try {
            ...
            ArrayList<Integer> overshotRecords = new ArrayList<>();
            HashSet<Integer> overshotSeen = new HashSet<>();
    ...
    private String extractBillingFromAltObis(String dataUpper,
        ...
        try {
            ...
            Map<String, Integer> altColMap = new LinkedHashMap<>();
    ...
    private Map<String, Integer> parseCaptureObjects(String dataUpper, String profileObis) {
        Map<String, Integer> colMap = new LinkedHashMap<>();
        try {
    ...
    private void patchAndReplaceBuffer(StringBuilder meterData, String dataUpper,
                                       String profileObis,
                                       Map<String, Integer> colMap, int adj) {
        try {
    ...
    private void postProcessMeterData(StringBuilder meterData) {
        try {
            ...
            Map<String, int[]> scalerMap = buildScalerMap(dataUpper);
            ...
            Map<String, int[]> billingScalerMapPost = buildBillingScalerMap(dataUpper);
            Map<String, int[]> adjScalerMap = billingScalerMapPost.isEmpty() ? scalerMap : billingScalerMapPost;

            // Detect energy scaler adjustment from billing (same firmware quirk applies to all profiles)
            Map<String, Integer> billingColMap = parseCaptureObjects(dataUpper, "0100620100FF");
            ...
            for (String obis : cumulativeProfiles) {
                ...
                Map<String, int[]> scalerForProfile = scalerMap;
                if (obis.equals("0100620100FF") && !billingScalerMapPost.isEmpty()) {
                    Map<String, int[]> merged = new HashMap<>(scalerMap);
            ...
            for (String obis : lpProfileOnly) {
                Map<String, Integer> colMap = parseCaptureObjects(dataUpper, obis);
                if (colMap.isEmpty()) continue;
    ...
    /**
     * V68 — reads a profile object's entries_in_use (attr=7) straight out of the TXT text.
    ...
    private List<String> findDlmsLines(String text, String linePrefix) {
    ...
    public void validateMeterDataForXML(StringBuilder meterData, String readingMode) {
        ...
        if (readingMode.equals("COMPLETE")) {
            ...
            List<String> euLines = findDlmsLines(dataStr, "0007 0100630100FF 07 ");
            ...
            List<String> bufLines = findDlmsLines(dataStr, "0007 0100630100FF 02 ");
        ...
        if (readingMode.contains("BILLING") || readingMode.equals("COMPLETE")) {
            ...
            // V68: hasBillingObj is an OR of the three, so a read carrying the capture-object
            ...
            if (hasBillingCaptureObj) {
                Map<String, Integer> bilColMap = parseBillingCaptureObjects(dataStr.toUpperCase());
    ...
    public void appendLog(String text) {
        ...
        String ts = new SimpleDateFormat("HH:mm:ss.SSS")
                .format(new Date());
    ...
    public void flushLog() {
        ...
        try {
            BufferedWriter bw = new BufferedWriter(
                    new FileWriter(currentLogPath, true));
    ...
    private void startDiagLog(String meterMakeLabel, String modeLabel, int lpDays,
        ...
        try {
            ...
            File[] _extDirs2 = context.getExternalMediaDirs();
...
*/
     ...
     */
    private void collectLpTimestampsLoose(String lpHex, TreeSet<Long> out) {
        if (lpHex == null || lpHex.isEmpty() || out == null) return;
        String lowerHex = lpHex.toLowerCase();
        int idx = 0;
        while (true) {
    ...
    private void lpAuditPage(String tag, String pageHex) {
        try {
            ...
            // V68: LOST= was logged and nothing acted on it, so a read that lost most of its
    ...
    // V59: maxHealRetries overload — GAPDRIVE (day-request already went fully silent) is a
    ...
    private int lpFillDayGaps(UsbSerialPort port, Date dayStart, int capturePeriodMin,
                              TreeSet<Long> daySlots,
                              List<String> lpPageHexList,
                              HashSet<String> seenPayloads, int dayIndex, int maxHealRetries) {
            ...
            if (!pageHex.isEmpty() && hasLoadProfileRecords(pageHex)) {
                ...
                if (!pageSlots.isEmpty() && pageSlots.last() < reqStart && desyncRetries < 4) {
                    ...
                    appendLog("RLS_GAP_DESYNC day=-" + dayIndex + " page=" + pages
                            + " stale=" + new Date(pageSlots.first())
                            + ".." + new Date(pageSlots.last())
                            + " reqFrom=" + new Date(reqStart)
                ...
                if (newCnt > 0) {
                    ...
                    TreeSet<Long> looseSlots = new TreeSet<>();
                    ...
                    if (!looseSlots.isEmpty() && looseSlots.last() < reqStart && desyncRetries < 4) {
                        ...
                        appendLog("RLS_GAP_DESYNC_MANGLED day=-" + dayIndex + " page=" + pages
                                + " stale=" + new Date(looseSlots.first())
                                + ".." + new Date(looseSlots.last())
                                + " reqFrom=" + new Date(reqStart)
                                + " retry=" + desyncRetries + " — purging link");
                        try {
                            abortPendingBlockTransfer(port);
    ...
    private StringBuilder ReadBillingData(UsbSerialPort port, ReadingMode readingMode)
    {
        ...
        // V63: guard against a link failure silently truncating the column list —
        // see the V63-SDK changelog entry at the top of this file for why this one
        ...
        if (readingMode == ReadingMode.BILLING) {
            try {
                ...
                int mm   = now.get(Calendar.MONTH) + 1;  // 1-based
    ...
    private String mergeLpPageHexList(List<String> pages) {
    ...
    /**
     * V68 — LP PLAIN FULL-READ FALLBACK.
     *
     ...
     * SDK could only estimate entry windows, and every estimate used the one access mode
    ...
    private StringBuilder ReadLoadSurveyData(UsbSerialPort port , int lsDays)
    {
        ...
        List<String> lpPageHexList = new ArrayList<>();
        int[] lpEntriesDeclared = {entriesInUse};
        HashSet<String> seenPayloads = new HashSet<>();
        int totalActualRecords = 0;
        // V60: declared here (not inside the "if (!bulkReadOk)" block below, where the
        ...
        if (!bulkReadOk) {
            ...
            if (entriesInUse == 0) {
                if (abortRequested) {
                    ...
                    relinkIfDirty(port, "LP-day-probes"); // V60
                    int dayProbeHealsBefore = linkHealCount; // V60
                    for (int probeDayOffset = 0; probeDayOffset >= -3 && !probeHadData; probeDayOffset--) {
                        Calendar probeCal = Calendar.getInstance();
                        probeCal.add(Calendar.DAY_OF_YEAR, probeDayOffset);
                        // Normalise to midnight — same fix as main day loop
                        probeCal.set(Calendar.HOUR_OF_DAY, 0);
                        probeCal.set(Calendar.MINUTE, 0);
                        probeCal.set(Calendar.SECOND, 0);
                        probeCal.set(Calendar.MILLISECOND, 0);
                        Date probeDate = probeCal.getTime();
                    ...
                    // V60: silence caused by a dirty/healed link taught us nothing about
            ...
            if ((entriesInUse > 0 || probeHadData) && !bulkFallbackUsed) {
                ...
                appendLog("LP: reading " + lsDays + " days | Target ~" + targetRecords
                        + " records | ETA ~" + etaStr);
                int stalledReconnectCount = 0; // V59: up to 2 full-HDLC reconnects per LP read (was 1)
                ...
                for (int i = 0; i <= lsDays; i++) {
                    ...
                    if (DLMdata != null && !DLMdata.toString().isEmpty()) {
                        ...
                        {
                            TreeSet<Long> selChk = new TreeSet<>();
                            ...
                            if (!selChk.isEmpty()
                                ...
                                appendLog("RLS_SEL_DAY_DESYNC day=-" + i
                                        + " stale=" + new Date(selChk.first())
                                        + ".." + new Date(selChk.last())
                        ...
                        if (!hasLoadProfileRecords(lpHex)) {
                            // V59: on a link that needed healing this session, "empty" days
                            ...
                            if (consecutiveEmptyDays >= ((linkHealCount > 0) ? 2 : 1)) { // V59: was 4/2 gated on i>3
                        ...
                        if (gapDriveDays < 10 && !abortRequested && !lpShouldAbort()) {
                            ...
                            int gdAdded = lpFillDayGaps(port, dayDate, capturePeriodMin,
                                    gdSlots, lpPageHexList, seenPayloads, i, 2); // V59: cheaper ladder — second-chance path
                            if (gdAdded > 0) {
                        ...
                        if (consecutiveEmptyDays >= ((linkHealCount > 0) ? 2 : 1)) { // V59: was 4/2 gated on i>3 — GAPDRIVE already exhausted its own heal-retry ladder for this day and still got added=0, so waiting for more fully-dead days before trying the real fix only guarantees more lost days
        ...
        if (!abortRequested && !lpShouldAbort()) {
            try {
                TreeSet<Long> allGot = new TreeSet<>();
                ...
                if (oldestGotMs > wantOldestMs + 86400000L) {
                    ...
                    while (hi >= 1 && entryPages < MAX_ENTRY_PAGES) {
                        ...
                        appendLog("LP_ENTRY_PAGE entries=" + lo + ".." + hi
                                ...
                                + (slots.isEmpty() ? "" : (" range=" + new Date(slots.first())
                                        + ".." + new Date(slots.last()))));
                        ...
                        if (!slots.isEmpty() && slots.first() <= wantOldestMs) {
                            appendLog("LP_ENTRY_TARGET_REACHED first=" + new Date(slots.first()));
                            break;
                        }
                        hi = lo - 1;
                    }
        ...
        if (!lpPageHexList.isEmpty()) {
            ...
            // V68: recovered by the plain full attr=2 read — see tryLpPlainFullRead().
            ...
            // V68: the plain full read above also failed, so selective access AND the
    ...
    private StringBuilder ReadMidnightSnapshot(UsbSerialPort port, int lsDays)
    {
        ...
        try {
            ...
            calStart.add(Calendar.DAY_OF_YEAR, -snapDays);
